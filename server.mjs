...
const schema = `
CREATE TABLE IF NOT EXISTS public.ai_agent_translations (
 agent_id UUID NOT NULL REFERENCES public.ai_agents(id) ON DELETE CASCADE,
 language_code TEXT NOT NULL REFERENCES public.languages(code),
 name TEXT NOT NULL,
 description TEXT NOT NULL DEFAULT '',
 PRIMARY KEY(agent_id,language_code)
);
...
async function audit(req,action,type,id,details={}){try{await pool.query("INSERT INTO public.audit_log(tenant_id,actor_auth_user_id,actor_admin_id,action,entity_type,entity_id,after_data) VALUES($1,$2,$3,$4,$5,$6,$7)",[null,null,req.session.admin.id,action,type,id,details]);}catch{}}
...
app.get("/api/agents",async(req,res)=>{
 const includeAll=req.query.include==='all' && req.session.admin;
 const q=await pool.query(`SELECT id,code,name,slug,description,default_language,supported_languages,voice_enabled,voice_provider,default_voice_id,default_voice_language,voice_settings,capabilities,status,is_public,sort_order FROM public.ai_agents ${includeAll?'':'WHERE status=\'published\' AND is_public=true'} ORDER BY sort_order,id`);
 const ids=q.rows.map(x=>x.id); let tr=[],vo=[]; if(ids.length){tr=(await pool.query("SELECT agent_id,language_code,name,description FROM public.ai_agent_translations WHERE agent_id=ANY($1::uuid[])",[ids])).rows;vo=(await pool.query("SELECT agent_id,language_code,provider,voice_id,voice_name,settings,is_default,is_active FROM public.ai_agent_voices WHERE agent_id=ANY($1::uuid[]) AND is_active=true ORDER BY is_default DESC",[ids])).rows;}
 res.json(q.rows.map(x=>{const translations={};for(const t of tr.filter(t=>t.agent_id===x.id))translations[t.language_code]={name:t.name,description:t.description};const voices={};for(const v of vo.filter(v=>v.agent_id===x.id))if(!voices[v.language_code])voices[v.language_code]=v;return {...x,translations,voices,title:{en:translations.en?.name||x.name,ar:translations.ar?.name||x.name,fr:translations.fr?.name||x.name},text:{en:translations.en?.description||x.description||'',ar:translations.ar?.description||x.description||'',fr:translations.fr?.description||x.description||''}};}));
});
app.post("/api/agents",auth,permit("owner","editor"),async(req,res)=>{const a=req.body||{},id=crypto.randomUUID(),slug=String(a.slug||a.code||id).toLowerCase().trim().replace(/[^a-z0-9-]+/g,'-');const client=await pool.connect();try{await client.query('BEGIN');await client.query("INSERT INTO public.ai_agents(id,tenant_id,code,name,slug,description,default_language,supported_languages,voice_enabled,status,is_public) VALUES($1,null,$2,$3,$4,$5,$6,$7,$8,'draft',false)",[id,a.code||`AGENT_${id.slice(0,8)}`,a.name||'New AI Agent',slug,a.description||'',a.default_language||'ar',a.supported_languages||['ar','en','fr'],a.voice_enabled!==false]);for(const l of ['ar','en','fr']){const n=a.translations?.[l]?.name||a.title?.[l]||a.name||'AI Agent';const d=a.translations?.[l]?.description||a.text?.[l]||a.description||'';await client.query("INSERT INTO public.ai_agent_translations(agent_id,language_code,name,description) VALUES($1,$2,$3,$4)",[id,l,n,d]);}await client.query('COMMIT');res.json((await pool.query("SELECT * FROM public.ai_agents WHERE id=$1",[id])).rows[0]);}catch(e){await client.query('ROLLBACK');res.status(400).json({error:e.message});}finally{client.release();}});
app.put("/api/agents/:id",auth,permit("owner","editor"),async(req,res)=>{const a=req.body||{},client=await pool.connect();try{await client.query('BEGIN');await client.query("UPDATE public.ai_agents SET code=$1,name=$2,slug=$3,description=$4,default_language=$5,supported_languages=$6,voice_enabled=$7,status=$8,is_public=$9,updated_at=now() WHERE id=$10",[a.code,a.name||'AI Agent',String(a.slug||a.code).toLowerCase().trim().replace(/[^a-z0-9-]+/g,'-'),a.description||'',a.default_language||'ar',a.supported_languages||['ar','en','fr'],a.voice_enabled!==false,a.status||'draft',a.is_public===true,req.params.id]);for(const l of ['ar','en','fr']){const n=a.translations?.[l]?.name||a.title?.[l]||a.name||'AI Agent',d=a.translations?.[l]?.description||a.text?.[l]||a.description||'';await client.query("INSERT INTO public.ai_agent_translations(agent_id,language_code,name,description) VALUES($1,$2,$3,$4) ON CONFLICT(agent_id,language_code) DO UPDATE SET name=EXCLUDED.name,description=EXCLUDED.description",[req.params.id,l,n,d]);}if(a.voices){for(const [l,v] of Object.entries(a.voices)){if(!v?.voice_id)continue;await client.query("INSERT INTO public.ai_agent_voices(agent_id,language_code,provider,voice_id,voice_name,is_default,is_active) VALUES($1,$2,$3,$4,$5,true,true) ON CONFLICT(agent_id,language_code,provider,voice_id) DO UPDATE SET is_default=true,is_active=true,voice_name=EXCLUDED.voice_name",[req.params.id,l,v.provider||a.voice_provider||'tts',v.voice_id,v.voice_name||'']);}}await client.query('COMMIT');res.json((await pool.query("SELECT * FROM public.ai_agents WHERE id=$1",[req.params.id])).rows[0]);}catch(e){await client.query('ROLLBACK');res.status(400).json({error:e.message});}finally{client.release();}});
app.get("/api/agents/:id/voices",auth,permit("owner","editor","media"),async(req,res)=>{const q=await pool.query("SELECT * FROM public.ai_agent_voices WHERE agent_id=$1 ORDER BY language_code",[req.params.id]);res.json(q.rows)});
app.delete("/api/agents/:id",auth,permit("owner"),async(req,res)=>{await pool.query("UPDATE public.ai_agents SET status='archived',is_public=false,updated_at=now() WHERE id=$1",[req.params.id]);await audit(req,'archive','agent',req.params.id);res.json({ok:true})});
app.get("/api/products",async(req,res)=>{const includeAll=req.query.include==='all' && req.session.admin;const q=await pool.query(`SELECT id,product_type,sku,name,slug,description,price,currency,status,is_digital,delivery_config,metadata FROM public.products ${includeAll?"":"WHERE status='published'"} ORDER BY created_at DESC`);res.json(q.rows);});
...
app.get("/api/audit",auth,permit("owner"),async(req,res)=>{const q=await pool.query("SELECT audit_log.*,admins.email FROM public.audit_log LEFT JOIN public.admins ON admins.id=audit_log.actor_admin_id ORDER BY audit_log.created_at DESC LIMIT 200");res.json(q.rows);});
